...
class CaseIntroductionTemplate(ConversationTemplateInterface):
    ...
    def get_introduction_prompt(self):
        return "Command: We just entered the start of the case interview. Start the case by introducing yourself to the candidate and stating the problem statement. After that the candidate can ask clarifying questions which you should answer using the provided reference information."
    
    def get_check_completion_prompt(self):  
        return f"""Command: Take the next response of the interviewee to evaluate if this part of the case study is completed. The section is completed if:
